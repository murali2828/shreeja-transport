...
- `plantAllocation.js` — "Plan to plant requirements" (DB-free): given the litres each plant requires, reassigns BMCUs from their catchment plant to short plants by lowest extra km × ₹/km per litre within a km limit (priority / proportional shortfall rule, pins, locked plants); the routing then runs on the allocation with plant switching off. `scripts/plant_allocation_selftest.js`.
- `dayOptimizerData.js` — builds the v2 instance from the DB: demand forecast (`bmcu_demand_forecast`), plant catchments, fleet availability (open Maintainance gate passes only, stale once the tanker ran again; planners exclude by hand), rate state per tanker, distance coverage, Google prefetch, comparison with actual plans.
