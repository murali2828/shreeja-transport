...
export const setMissedBmcuRemark   = (data) => api.put('/executions/coverage/missed-remark', data);
...
export const printTripDoc     = (planId, doc_type, printed_at, no_print) => api.post(`/trip-docs/${planId}/print`, { doc_type, printed_at: printed_at || undefined, no_print: no_print || undefined });
...
export const getTrackingBmcus     = ()  => api.get('/tracking/bmcus');
export const getTrackingTrips     = (p) => api.get('/tracking/trips', { params: p });
export const getTrackingTrip      = (executionId, p) => api.get(`/tracking/trip/${executionId}`, { params: p });
export const getTrackingTripReport  = (executionId) => api.get(`/tracking/trip/${executionId}/report`, { responseType: 'blob' });
export const getTrackingFleetReport = (p) => api.get('/tracking/report', { params: p, responseType: 'blob' });
...
export const sendDailyReport    = (date, basis = 'plan') => api.post('/reports/send-email', { report_date: date, date_basis: basis });
export const downloadTSExcel    = (date, basis = 'plan') =>
  api.get(`/reports/daily-ts/excel?report_date=${date}&date_basis=${basis}`, { responseType: 'blob' }).then(r => {
    const url = URL.createObjectURL(r.data);
    const a = document.createElement('a');
